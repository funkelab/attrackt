...
def preallocate_memory(dataset, model_lightning, batch_size, max_tokens, device):
    ...
    if max_tokens is None:
        ...
        batch = dict(
            ...
            labels=batched(
                torch.zeros(max_len, dtype=x["labels"].dtype), batch_size, device
            ),
            **(
                {
                    "autoencoder_features": batched(
                        torch.zeros(
                            (max_len,) + x["autoencoder_features"].shape[1:],
                            dtype=x["autoencoder_features"].dtype,
                        ),
                        batch_size,
                        device,
                    )
                }
                if "autoencoder_features" in x
                else {}
            ),
            **(
                {
                    "assoc_matrix_mask": batched(
                        torch.zeros(
                            (max_len, max_len),
                            dtype=x["assoc_matrix_mask"].dtype,
                        ),
                        batch_size,
                        device,
                    )
                }
                if "assoc_matrix_mask" in x
                else {}
            ),
            **(
                {
                    "assoc_matrix": batched(
                        torch.zeros(
                            (max_len, max_len),
                            dtype=x["assoc_matrix"].dtype,
                        ),
                        batch_size,
                        device,
                    )
                }
                if "assoc_matrix" in x
                else {}
            ),
        )
