...
from attrackt.trackastra.data.utils import load_tiff_timeseries
...
def get_features(
    ...
    print(f"ndim being used is {ndim}.")
    detections = _check_dimensions(detections, ndim)
    imgs = _check_dimensions(imgs, ndim)
    logger.info(f"Extracting features from {len(detections)} time frames.")
    if n_workers > 0:
