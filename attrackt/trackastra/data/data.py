...
class CTCData(Dataset):
    def __init__(
        self,
        embeddings_directory: str | None = None,
        root: str = "",
        ndim: int = 2,
        use_gt: bool = True,
        ...
        finetuning_embeddings: bool = False,
        augment: int = 2,
        features: Literal[
            "none",
            "regionprops",
        ...
        **kwargs,
        ...
        logging.info(
            f"Name of the sequence: {root}."
        )  # This would be the name of the Sequence
        self.root = Path(root)
        self.name = self.root.name
        self.finetuning_embeddings = finetuning_embeddings

        self.embeddings_dictionary = None
        if embeddings_directory is not None:
            self.embeddings_csv_file_name = embeddings_directory / (self.name + ".csv")
            logging.info(f"Embeddings_csv_file_name {self.embeddings_csv_file_name}.")
            if self.embeddings_csv_file_name.exists():
                logging.info("Embedding name exists.")
                embeddings_data = np.loadtxt(
                    self.embeddings_csv_file_name, delimiter=" "
                )
                self.embeddings_dictionary = {}
                for row in embeddings_data:
                    id_, t = int(row[0]), int(row[1])
                    node_id = str(t) + "_" + str(id_)
                    self.embeddings_dictionary[node_id] = row[2:]
    ...
    def _build_windows(
        ...
        for t1, t2 in tqdm(
            ...
            if self.embeddings_dictionary is not None:
                autoencoder_features = []
                for time, label in zip(_ts, _labels):
                    node_id = str(time) + "_" + str(label)
                    autoencoder_features.append(self.embeddings_dictionary[node_id])

                autoencoder_features = np.asarray(autoencoder_features)
            ...
            if "autoencoder_features" in locals():
                w = dict(
                    coords=_coords,
                    t1=t1,
                    img=self.imgs[t1:t2],
    ...
    def __getitem__(self, n: int, return_dense=None):
        ...
        min_time = track["t1"]
        if "autoencoder_features" in track:
            autoencoder_features = track["autoencoder_features"]
        ...
        if self.cropper is not None:
            ...
            if len(idx) > 0:
                ...
                if "autoencoder_features" in locals():
                    autoencoder_features = autoencoder_features[idx]
            else:
        ...
        if "autoencoder_features" in locals():
            autoencoder_features = torch.from_numpy(autoencoder_features).float()

        if self.augmenter is not None:
            coords = coords0.clone()  # (N, 3)
    ...
    def _build_windows_wrfeat(
        ...
        for t1, t2 in tqdm(
            ...
            img = self.imgs[t1:t2]  # (6, H, W)
            mask = det_masks[t1:t2]
            feat = wrfeat.WRFeatures.concat(features[t1:t2])

            labels = feat.labels  # (N,)
            timepoints = feat.timepoints  # (N,)
            coords = feat.coords  # (N, 2)

            if (
                self.embeddings_dictionary is not None
                and not self.finetuning_embeddings
            ):
                autoencoder_features = []
                for time, label in zip(timepoints, labels):
                    node_id = str(time) + "_" + str(label)
                    autoencoder_features.append(self.embeddings_dictionary[node_id])
                autoencoder_features = np.asarray(autoencoder_features)
            elif self.embeddings_dictionary is not None and self.finetuning_embeddings:
                autoencoder_features = []
                min_time = timepoints.min()
                timepoints_scaled = timepoints - min_time
                img_padded = np.pad(img, ((0, 0), (32, 32), (32, 32)), mode="constant")
                for time, label, coord in zip(timepoints_scaled, labels, coords):
                    tly = int(coord[0] - 32) + 32  # plus 32 for padding
                    tlx = int(coord[1] - 32) + 32  # plus 32 for padding
                    img_crop = img_padded[int(time), tly : tly + 64, tlx : tlx + 64]
                    img_crop_ravel = img_crop.ravel()  # should be 64*64
                    autoencoder_features.append(img_crop_ravel)
                autoencoder_features = np.asarray(autoencoder_features)
...
def collate_sequence_padding(max_len: int | None = None):
    ...
    def collate_sequence(batch):
        ...
        if (
            "autoencoder_features" in batch[0]
        ):  # Assumes all items in batch have the same structure
            normal_keys["autoencoder_features"] = 0  # Define padding value if needed
