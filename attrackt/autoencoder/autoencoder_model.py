...
from attrackt.autoencoder.decoder import Decoder
from attrackt.autoencoder.diagonal_gaussian_distribution import (
    DiagonalGaussianDistribution,
)
from attrackt.autoencoder.encoder import Encoder
...
class AutoencoderModel(nn.Module):
    def __init__(
        ...
        self.post_quant_conv = torch.nn.Conv2d(embedding_dimension, num_z_channels, 1)
    ...
    def forward(
        ...
        """forward.
        This function takes in a torch tensor (B, C, H, W).
        Generates an embedding of the image using the encoder. (B, z, h, w)
        ...
        im : torch.Tensor
            im
        only_encode : bool
            If set to True, only the encoded embeddings are returned.
        ...
        """
        if self.variational:
            hidden = self.encoder(im)  # B 2z h w
            moments = self.pre_quant_conv(hidden)  # B 2e h w
            posterior = DiagonalGaussianDistribution(parameters=moments)
            if only_encode:
                posterior_mean = posterior.get_mean()  # B e h w
                posterior_mean = posterior_mean.view(posterior_mean.shape[0], -1)
                return posterior_mean  # B ehw
            else:
                z = posterior.rsample()  # B e h w
                y = self.decoder(self.post_quant_conv(z))  # B C H W
                return y, posterior
        else:
            embeddings = self.encoder(im)  # B z h w
            if only_encode:
                embeddings = embeddings.view(embeddings.shape[0], -1)
                return embeddings  # B zhw
            else:
                y = self.decoder(embeddings)  # B C H W
                return y
